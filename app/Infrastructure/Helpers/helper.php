<?php
    if(!function_exists('random_string_generator')){
        function random_string_generator(){
            return substr(str_shuffle(str_repeat('abcdefghijklmnopqrstuvwxyz0123456789', 10)), 0, 20);
        }
    }

    if(!function_exists('is_assoc')){
        function is_assoc($array){
            $keys = array_keys($array);
            return $keys !== array_keys($keys);
        }
    }

    if(!function_exists('point_belongs_to_polygon')){
        function point_belongs_to_polygon($numberOfPoints, $latPoints, $lngPoints, $latUser, $lngUser){
            $i = $j = $c = 0;
            for ($i = 0, $j = $numberOfPoints-1 ; $i < $numberOfPoints; $j = $i++) {
            if ( (($lngPoints[$i] > $lngUser != ($lngPoints[$j] > $lngUser)) &&
              ($latUser < ($latPoints[$j] - $latPoints[$i]) * ($lngUser - $lngPoints[$i]) / ($lngPoints[$j] - $lngPoints[$i]) + $latPoints[$i]) ) ) 
                    $c = !$c;
            }
            return $c;
        }
    }

    if(!function_exists('_dd')){
        function _dd(...$args)
        {
            $content = '<span>';
            ob_start();
            dump(...$args);
            $content .= ob_get_contents();
            ob_end_clean();
            $content .= '</span>';
            $content = str_replace(['<div', '</div>'], ['<span', '</span>'], $content);
            response()->make($content, 500, ['Content-Type' => 'text/html'])->send();
            die(1);
        }
    }